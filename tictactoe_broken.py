...
def check_space_empty(game, row, column):
    return game[row][column] == 0 # Should not add 1 with colum otherwise it'll check the next column instead of the current one.

def convert_input_to_coordinate(user_input):
    return user_input - 1

def switch_player(player):
    if player == 1: # Should be ==
        return 2
    else:
        return 1
...
if __name__ == '__main__':
    ...
    while winner == 0 and moves_exist(game):
        ...
        while not available:
            ...
            while (row <= -1 or row > 2): # While loop to get the right input.
                row = convert_input_to_coordinate(int(input("Which row? (start with 1) ")))
            column = convert_input_to_coordinate(int(input("Which column? (start with 1) ")))
            while (column <= -1 or column > 2): # While loop to get the right input.
                column = convert_input_to_coordinate(int(input("Which column? (start with 1) ")))
            available = check_space_empty(game, row, column) # The column argument was missing.
